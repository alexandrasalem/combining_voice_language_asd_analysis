---
title: "ALM + AVM analysis"
author: "Alexandra Salem and Heather MacFarlane"
output:
...
raw_friends_voice_alm_data %>% filter(diagnosis_binary == "ASD") %>% count(ps_childethnicity)
raw_friends_voice_alm_data %>% filter(diagnosis_binary == "ASD") %>% count(race_eric)
...
```{r}
fisher.test(x = raw_friends_voice_alm_data$child_gender, y = raw_friends_voice_alm_data$diagnosis_binary)
```
...
```{r}
roc_data_plot <- (
  data.frame("pred" = friends_speech_predictions$speech_prob_asd, "diag" = friends_speech_predictions$diagnosis_binary, "Model" = "AVM\nAUC = 0.7800\n")) %>% bind_rows(
...
## mSVM-RFE feature selection
This section uses this source code: https://github.com/johncolby/SVM-RFE

Full generalization error:
...
results = lapply(folds, svmRFE.wrap, friends_voice_alm_data %>% select(diagnosis_binary, everything(), -ASDID, -num_utts_total, -total_num_words_fluent_intelligible, -activity_time), k=10)
```
```{r}
top.features = WriteFeatures(results, friends_voice_alm_data %>% select(diagnosis_binary, everything(), -ASDID, -num_utts_total, -total_num_words_fluent_intelligible, -activity_time), save=F)
```

```{r}
featsweep = lapply(1:17, FeatSweep.wrap, results, friends_voice_alm_data %>% select(diagnosis_binary, everything(), -ASDID, -num_utts_total, -total_num_words_fluent_intelligible, -activity_time))
```


```{r}
no.info = min(prop.table(table((friends_voice_alm_data %>% select(diagnosis_binary, everything(), -ASDID, -num_utts_total, -total_num_words_fluent_intelligible, -activity_time))[,1])))
...
set.seed(42)
z <- lapply(1:k, function(x){
  model = svm(diagnosis_binary~ content_ratio + c_units_per_minute + RMS + F0 + MLUM + Shimmer + um_ratio + LogSpectralEntropy, data = friends_voice_alm_data[folds != x, ], kernel = "linear", scale = FALSE, cost = 100, probability = T)
  pred <- predict(model, friends_voice_alm_data[folds == x, ])
  true <- friends_voice_alm_data$diagnosis_binary[folds == x]
  prob_asd <- attr(predict(model, friends_voice_alm_data[folds == x, ], probability = T), "probabilities")[,1]
  return(data.frame(pred = pred, true = true, prob_asd = prob_asd))
...
```{r}
pred_combined_small_best <- do.call(rbind, z)
caret::confusionMatrix(pred_combined_small_best$pred, pred_combined_small_best$true)
```

```{r}
roc(response = pred_combined_small_best$true, predictor = pred_combined_small_best$prob_asd, ci = T)
```

```{r}
roc.test(roc(response = pred_combined_ten$true, predictor = pred_combined_ten$prob_asd, ci = T), roc(response = pred_combined_small_best$true, predictor = pred_combined_small_best$prob_asd, ci = T))
```
